...
ReclamRepository recrep; 
	...
	public List<Reclam> getReclamByType(String ch) {
		// TODO Auto-generated method stub
		return recrep.ListReclam(ch);
	}
	
	public Reclam reclamupdatereclam(  Reclam rec)
	{
		Integer idRec=rec.getIdRec(); 
		Reclam reclam=recrep.findById(idRec).get();
		reclam.setDescriptionRec(rec.getDescriptionRec());
		reclam.setTypeRec(rec.getTypeRec());
		return recrep.save(reclam); 
	}
}
